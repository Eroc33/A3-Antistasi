...
if ((!isMultiPlayer) and foundACEMedical) then {_mal = true};
if (_mal) then
	{
	if (isServer) then
		{
		...
		if ((!isMultiPlayer) and foundACEMedical) then {hint "ACE medical is not supported in SP, please restart Arma and Antistasi without it"};
		}
	else
		{
