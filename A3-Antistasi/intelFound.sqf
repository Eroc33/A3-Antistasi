...
if (count _this == 1) then
	...
	if (_marcador isEqualType "") then
		{
		if (_marcador in airports) then {_chance = 30} else {_chance = 15};
		if (sides getVariable [_marcador,sideUnknown] == veryBad) then {_lado = veryBad};
		}
	else
		{
...
_minasAAF = allmines - (detectedMines good);
if (_lado == bad) then {_minasAAF = _minasAAF - (detectedMines veryBad)} else {_minasAAF = _minasAAF - (detectedMines bad)};
_revelaMina = false;
if (count _minasAAF > 0) then
	{
	{if (random 100 < _chance) then {good revealMine _x; _revelaMina = true}} forEach _minasAAF;
	};
