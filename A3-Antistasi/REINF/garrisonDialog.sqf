...
if (not(sides getVariable [_cercano,sideUnknown] == good)) exitWith {hint format ["That zone does not belong to %1",nameBuenos]; _nul=CreateDialog "build_menu";};
...
if (_tipo == "rem") then
	...
	if (_puestoFIA) then
		...
		sides setVariable [_cercano,nil,true];
		}
	else
		{
