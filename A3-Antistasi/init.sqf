...
if (!isMultiPlayer) then
    ...
    initVar = true;
    respawnBad setMarkerAlpha 0;
    "respawn_east" setMarkerAlpha 0;
    [] execVM "briefing.sqf";
    diag_log format ["Antistasi SP. InitVar done. Version: %1",antistasiVersion];
    _nul = [] execVM "musica.sqf";
    {if (/*(side _x == good) and */(_x != comandante) and (_x != Petros)) then {_grupete = group _x; deleteVehicle _x; deleteGroup _grupete}} forEach allUnits;
    ...
    switchCom = false;
    tkPunish = false;
    distanciaMiss = if (foundIFA) then {2000} else {4000};
    skillMult = 1;
    minWeaps = 24;
    civTraffic = 1;
    limitedFT = false;
    ...
    if (good == independent) then {deleteMarker "respawn_west"} else {deleteMarker "respawn_guerrila"};
    };
