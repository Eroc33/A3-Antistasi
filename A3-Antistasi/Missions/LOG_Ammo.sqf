...
_tipoVeh = if (_lado == bad) then {vehNATOAmmoTruck} else {vehCSATAmmoTruck};
...
[[good,civilian],"LOG",[format ["We've spotted an Ammotruck in an %1. Go there and destroy or steal it before %2:%3.",_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"Steal or Destroy Ammotruck",_marcador],_pos,false,0,true,"rearm",true] call BIS_fnc_taskCreate;
_camionCreado = false;
missions pushBack ["LOG","CREATED"]; publicVariable "missions";

waitUntil {sleep 1;(dateToNumber date > _fechalimnum) or ((spawner getVariable _marcador != 2) and !(sides getVariable [_marcador,sideUnknown] == good))};
_bonus = if (_dificil) then {2} else {1};
if ((spawner getVariable _marcador != 2) and !(sides getVariable [_marcador,sideUnknown] == good)) then
	{
	//sleep 10;
	...
	_camionCreado = true;
	if (_lado == bad) then {[_camion] call A3A_fnc_NATOcrate} else {[_camion] call A3A_fnc_CSATcrate};
	...
	_tipoGrupo = if (_dificil) then {if (_lado == bad) then {NATOSquad} else {CSATSquad}} else {if (_lado == bad) then {groupsNATOSentry} else {groupsCSATSentry}};
	//_cfg = if (_lado == bad) then {cfgNATOInf} else {cfgCSATInf};
	...
	waitUntil {sleep 1; (not alive _camion) or (dateToNumber date > _fechalimnum) or ({(_x getVariable ["spawner",false]) and (side group _x == good)} count crew _camion > 0)};
	...
	if ((not alive _camion) or ({(_x getVariable ["spawner",false]) and (side group _x == good)} count crew _camion > 0)) then
		{
		if ({(_x getVariable ["spawner",false]) and (side group _x == good)} count crew _camion > 0) then
			{
...
if (_camionCreado) then
	...
	deleteGroup _grupo1;
	deleteMarker _mrk;
	waitUntil {sleep 1; !([300,1,_camion,good] call A3A_fnc_distanceUnits)};
