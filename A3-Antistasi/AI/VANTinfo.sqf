...
_enemigosS = if (_lado == veryBad) then {bad} else {veryBad};
...
while {alive _veh} do
	...
	_conocidos = allUnits select {((side _x == good) or (side _x == _enemigosS)) and (alive _x) and (_x distance _posicion < 500)};
	{
	_grupo = _x;
		{
