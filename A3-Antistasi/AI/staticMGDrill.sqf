...
private _tipoVeh = 	if !(_esMortero) then
						{
						if (_lado == bad) then {NATOMG} else {if (_lado == veryBad) then {CSATMG} else {SDKMGStatic}};
						}
					else
						{
						if (_lado == bad) then {NATOMortar} else {if (_lado == veryBad) then {CSATMortar} else {SDKMortar}};
						};
